...
// POST - Recibir una orden de compra (solo ADMIN)
export async function POST(request: NextRequest, { params }: Parametros) {
  ...
  try {
    ...
    const orden = await OrdenesCompraService.recibir(ordenId, ip ?? '127.0.0.1', empresaId);
    ...
    return NextResponse.json({ error: 'Error al recibir la orden de compra' }, { status: 500 });
  }
}
